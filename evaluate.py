...
def validate_rubric(rubric):
    ...
    if not isinstance(rubric.get("model", "jev-latest"), str) or not rubric.get("model", "jev-latest").strip():
    ...
    for name, dimension in dimensions.items():
        ...
        if not isinstance(levels, list) or not 2 <= len(levels) <= 10 or not all(isinstance(x, str) and x.strip() for x in levels):
            raise EvaluationError(f"{name}: supply 2–10 nonempty level descriptions, ordered worst to best.")
        if not isinstance(dimension.get("instructions"), str) or not dimension["instructions"].strip():
            raise EvaluationError(f"{name}: instructions are required.")
        if not number(dimension.get("weight", 1), 0, 1e6):
            raise EvaluationError(f"{name}: weight must be finite and between 0 and 1000000.")
...
def documents(inputs):
    ...
    for value in inputs:
        path = Path(value)
        if path.is_dir():
            found.extend(sorted(p for p in path.rglob("*") if p.is_file() and p.suffix.lower() in SUPPORTED))
        elif path.is_file() and path.suffix.lower() in SUPPORTED:
            found.append(path)
        else:
            raise EvaluationError(f"Not a supported document or directory: {path}. Use .txt or .md files.")
...
def payload_for(path, rubric):
    if path.stat().st_size > 1_000_000:
        raise EvaluationError(f"{path.name}: file exceeds the 1 MB input limit; split it explicitly.")
    content = path.read_text(encoding="utf-8")
    if not content.strip() or "\x00" in content:
        raise EvaluationError(f"{path.name}: expected nonempty UTF-8 text.")
    payload = {
        "model": rubric.get("model", "jev-latest"),
        "state": {"document": {"name": path.name, "text": content}, "context": rubric.get("context", "")},
        "questions": {name: {"type": "score", "instructions": d["instructions"], "criteria": d["levels"]}
    ...
    if len(json.dumps(payload, ensure_ascii=False).encode("utf-8")) > 28_000:
        raise EvaluationError(f"{path.name}: request exceeds the conservative 28000-byte budget; split the document or shorten the rubric.")
    return payload
    ...
    for attempt in range(3):
        try:
            ...
            if status in (429, 500, 502, 503, 504, 529) and attempt < 2:
                ...
                if delay > 60:
                    raise EvaluationError(f"TypeSafe HTTP {status}: retry after {delay:g} seconds.") from None
                time.sleep(delay)
                continue
            hint = "Check TYPESAFE_API_KEY (an OpenRouter key is not a TypeSafe key)." if status in (401, 403) else "Check the request, model, quota, and TypeSafe service status."
            raise EvaluationError(f"TypeSafe HTTP {status}. {hint}") from None
        except (urllib.error.URLError, TimeoutError, OSError):
            raise EvaluationError("TypeSafe connection failed or timed out; check your network and retry.") from None
...
def summarize(response, rubric):
    ...
    for name, d in rubric["dimensions"].items():
        ...
        if not isinstance(answer, dict) or answer.get("type") != "score" or not number(answer.get("score"), 0, maximum) or not number(answer.get("confidence"), 0, 1):
            raise EvaluationError(f"Invalid or missing score answer for {name}.")
        probabilities = answer.get("probabilities")
        if (not isinstance(probabilities, dict) or set(probabilities) != {str(i) for i in range(maximum + 1)}
        ...
        dimensions[name] = {**answer, "score_percent": normalized, "weight": d.get("weight", 1), "needs_review": needs_review}
        weighted += normalized * d.get("weight", 1)
        total_weight += d.get("weight", 1)
        review |= needs_review
    return {"model": response.get("model"), "usage": response.get("usage"), "dimensions": dimensions,
            "composite_percent": weighted / total_weight, "needs_review": review}
...
def render(report, format):
    ...
    writer.writerow(["document", "dimension", "score_percent", "confidence", "composite_percent", "needs_review", "error"])
    for result in report["results"]:
        if "error" in result:
            writer.writerow([result["document"], "", "", "", "", "", result["error"]])
        else:
            for name, d in result["dimensions"].items():
                writer.writerow([result["document"], name, d["score_percent"], d["confidence"], result["composite_percent"], d["needs_review"], ""])
    return output.getvalue()


def main(argv=None):
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("documents", nargs="+", help="UTF-8 .txt/.md files or directories (recursive)")
    parser.add_argument("--rubric", required=True, help="JSON rubric with dimensions and ordered levels")
    parser.add_argument("--dry-run", action="store_true", help="Print exact request bodies without API calls or keys")
    parser.add_argument("--prompt-key", action="store_true", help="Prompt for a hidden TypeSafe key; never save it")
    parser.add_argument("--format", choices=["json", "csv"], default="json")
    parser.add_argument("--output", type=Path, help="Write a new report file (refuses to overwrite)")
    ...
    try:
        ...
        if args.dry_run:
            text = json.dumps({"endpoint": ENDPOINT, "requests": [p for _, p in prepared]}, indent=2, ensure_ascii=False) + "\n"
            failed = False
        else:
            ...
            for path, payload in prepared:
                ...
                result = {"document": str(path), "sha256": hashlib.sha256(payload["state"]["document"]["text"].encode()).hexdigest()}
                try:
                    result.update(summarize(request_evaluation(payload, key.strip()), rubric))
