...
def make_payload(config, text, model="jev-latest"):
    ...
    if len(json.dumps(payload, ensure_ascii=False).encode("utf-8")) > 28_000:
        raise EvaluationError(
            "Input exceeds the conservative 28000-byte request budget; shorten it."
        )
    return payload
...
def parse_response(response, config, threshold):
    ...
    if (
        ...
        or applicable.get("type") != "noul"
        or not number(applicable.get("noul"), 0, 1)
    ):
        raise EvaluationError("TypeSafe returned an invalid applicability answer.")
    reasons = []
    if applicable["noul"] < 0.8:
        reasons.append(
            "Input may be outside this example's scope or missing necessary facts."
        )
    ...
    return {
        ...
        "applicable_probability": applicable["noul"],
...
def display(result):
    ...
    lines = [
        f"{label}: {result['choice'].replace('_', ' ').title()}",
...
def main(argv=None):
    ...
    source.add_argument(
        "--text", help="Feedback or a game situation; prompts if omitted"
    )
    source.add_argument(
        "--sample", type=int, choices=[1, 2, 3], help="Use a built-in sample"
    )
    parser.add_argument(
        "--list-samples",
        action="store_true",
        help="Print sample inputs without API calls",
    )
    parser.add_argument(
        "--dry-run", action="store_true", help="Print the request without using a key"
    )
    parser.add_argument(
        "--prompt-key",
        action="store_true",
        help="Enter a hidden TypeSafe key without saving it",
    )
    ...
    try:
        ...
        if text is None:
            if sys.stdin.isatty():
                print(
                    f"{config['title']} — enter your input: ",
                    end="",
                    file=sys.stderr,
                    flush=True,
                )
