...
class ChoiceTests(unittest.TestCase):
    def setUp(self):
        ...
        self.response = {"model": "jev-test", "usage": {}, "answers": {
            "decision": {"type": "choice", "choice": "bug_report", "confidence": 0.9,
                         "probabilities": {"bug_report": 0.94, "feature_request": 0.04, "praise": 0.02}},
    ...
    def test_out_of_scope_and_uncertainty_require_review(self):
        self.assertFalse(choose.parse_response(self.response, self.config, 0.5)["needs_review"])
        ...
        self.assertTrue(choose.parse_response(self.response, self.config, 0.5)["needs_review"])

    def test_rejects_invalid_answers(self):
        for key, value in [("choice", []), ("choice", "other"), ("choice", "praise"), ("confidence", float("nan")),
                           ("probabilities", {"bug_report": 1}), ("probabilities", {"bug_report": True, "feature_request": 0, "praise": 0})]:
    ...
    def test_dry_run_and_samples_never_use_key_or_network(self):
        for example in ["feedback", "game"]:
            with patch.dict("os.environ", {}, clear=True), patch.object(choose, "request_evaluation") as request, patch("getpass.getpass") as prompt, contextlib.redirect_stdout(io.StringIO()) as output:
                self.assertEqual(choose.main([example, "--sample", "1", "--dry-run", "--prompt-key"]), 0)
                json.loads(output.getvalue())
                request.assert_not_called()
                prompt.assert_not_called()
            with patch.object(choose, "request_evaluation") as request, contextlib.redirect_stdout(io.StringIO()):
                self.assertEqual(choose.main([example, "--list-samples"]), 0)
                request.assert_not_called()

    def test_cli_json_with_mocked_service(self):
        with patch.dict("os.environ", {"TYPESAFE_API_KEY": "test-key"}), patch.object(choose, "request_evaluation", return_value=self.response) as request, contextlib.redirect_stdout(io.StringIO()) as output:
            self.assertEqual(choose.main(["feedback", "--text", "Export crashes", "--json"]), 0)
    ...
    def test_missing_key_and_api_errors(self):
        ...
        with patch.dict("os.environ", {"TYPESAFE_API_KEY": "test"}), patch.object(choose, "request_evaluation", side_effect=choose.EvaluationError("Unavailable")), contextlib.redirect_stderr(io.StringIO()):
            self.assertEqual(choose.main(["game", "--sample", "1"]), 2)

    def test_piped_input(self):
        with patch("sys.stdin", io.StringIO("Please add dark mode\n")), contextlib.redirect_stdout(io.StringIO()) as output:
            self.assertEqual(choose.main(["feedback", "--dry-run"]), 0)
        self.assertEqual(json.loads(output.getvalue())["request"]["state"]["input"], "Please add dark mode")
